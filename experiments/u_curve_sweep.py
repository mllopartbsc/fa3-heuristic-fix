...
Uses kernel-level timing with precomputed scheduler metadata (same as main_results)
so the figure aligns with Table 1 and shows the s=1 vs s>=3 improvement.
"""
...
def run(*, track: str, quick: bool = False, results_dir=None):
    ...
    print(f"Kernel-level sweep for B={B}, H_KV={H_KV}, L_K={L_K} (MQA boundary)")
    print("Measuring kernel latency with precomputed scheduler metadata (CUDA Graph)\n")

    splits_to_test = list(range(1, 65))
    results = []
    ...
    header = f"{'Splits':<8} {'Kernel Latency (us)'}"
    print(header)
    print("-" * 35)
    ...
    for s in splits_to_test:
        ...
        results.append({
            "splits": s,
            "service_latency_us": round(lat, 2),  # keep field name for plot script
        })
    ...
    out_path = write_track_json(
        ...
        benchmark_mode="manual_split_kernel_metadata",
    )
